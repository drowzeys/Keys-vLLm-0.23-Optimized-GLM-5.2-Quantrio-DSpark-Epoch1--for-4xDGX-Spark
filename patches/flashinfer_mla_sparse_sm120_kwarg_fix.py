...
try:
    from flashinfer.decode import trtllm_batch_decode_with_kv_cache_mla as _t
    _TRTLLM_MLA_HAS_KV_SCALE_FORMAT = (
        "kv_scale_format" in _inspect.signature(_t).parameters
    )
except Exception:
    _TRTLLM_MLA_HAS_KV_SCALE_FORMAT = False
...
class FlashInferMLASparseSM120Impl(SparseMLAAttentionImpl[FlashInferMLASparseMetadata]):
    ...
    def forward_mqa(
        ...
        out = flashinfer_trtllm_batch_decode_with_kv_cache_mla(
            ...
            # GB10/flashinfer-0.6.12: kv_scale_format is a 0.6.13+ kwarg; the
            # 0.6.12 default matches the fp8_ds_mla layout (DSV4F-proven here).
            **(
                {"kv_scale_format": self.kv_scale_format}
                if _TRTLLM_MLA_HAS_KV_SCALE_FORMAT
                else {}
            ),
        )
