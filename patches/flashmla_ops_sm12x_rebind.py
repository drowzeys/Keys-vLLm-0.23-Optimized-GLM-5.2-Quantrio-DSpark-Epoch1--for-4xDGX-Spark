...
def is_flashmla_sparse_supported() -> tuple[bool, str | None]:
    ...
    if not (
        ...
        or current_platform.is_device_capability_family(120)  # GLM52_B12X_SPARSE
    ):
        return (
            False,
...
# --- GLM52_SM12X_FLASHMLA (GB10): bind portable Triton sparse ops on cap family 120.
# The native _flashmla_C extension is sm90/100-only; the sm12x Triton drop-ins
# (verified byte-exact on the 448-NoPE/584B blocked layout) take over here.
def _glm52_sm12x_flashmla_rebind() -> None:
    global flash_mla_sparse_fwd, flash_mla_with_kvcache, get_mla_metadata
    try:
        ...
        return
    if _flashmla_C_AVAILABLE:
        return
    from vllm.v1.attention.backends.mla.sm12x_sparse_mla_attn import (
        flash_mla_sparse_fwd_triton,
        flash_mla_with_kvcache_triton,
    )
    flash_mla_sparse_fwd = flash_mla_sparse_fwd_triton
    flash_mla_with_kvcache = flash_mla_with_kvcache_triton

    def _get_mla_metadata_sm12x(*_args, **_kwargs):
        # Triton path schedules internally; (tile_scheduler_metadata, num_splits)
        return None, None

    get_mla_metadata = _get_mla_metadata_sm12x


_glm52_sm12x_flashmla_rebind()
